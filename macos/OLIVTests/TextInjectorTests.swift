...
final class TextInjectorTests: XCTestCase {
    ...
    // Undo-last-dictation: Cmd+Z posts when Accessibility is granted and
    // secure input is off. Pasteboard is untouched.
    func testUndoPostsCmdZ() {
        let injector = TextInjector(pasteboard: pasteboard)
        var zCalls = 0
        injector.postAccessCheck = { true }
        injector.secureInputCheck = { false }
        injector.cmdZPoster = { zCalls += 1; return true }
        pasteboard.clearContents()
        ...
        XCTAssertTrue(result.posted)
        XCTAssertFalse(result.usedFallback)
        XCTAssertEqual(zCalls, 1)
        XCTAssertEqual(currentString(), "KEEP", "undo must not touch the clipboard")
    }

    func testUndoSecureInputFallsBack() {
        let injector = TextInjector(pasteboard: pasteboard)
        var zCalls = 0
        injector.postAccessCheck = { true }
        injector.secureInputCheck = { true }
        injector.cmdZPoster = { zCalls += 1; return true }

        let result = injector.undo()
        XCTAssertFalse(result.posted)
        XCTAssertTrue(result.usedFallback)
        XCTAssertEqual(zCalls, 0)
    }

    func testUndoMissingAccessibilityFallsBack() {
        let injector = TextInjector(pasteboard: pasteboard)
        var zCalls = 0
        injector.postAccessCheck = { false }
        injector.secureInputCheck = { false }
