...
final class HotkeyRecorderTests: XCTestCase {
    ...
    // Chord capture: modifiers accumulate; the main key commits Control+Option+F17.
    // Control-down alone must NOT capture (the bug that made bare Ctrl fire PTT).
    func testCaptureSessionBuildsCtrlAltF17Chord() {
        var s = HotkeyCaptureSession()
        XCTAssertEqual(
            s.handle(event: .flagsChanged, keyCode: 0x3B, modifierIsDown: true, characters: nil),
            .ignore)  // Left Control down — wait
        XCTAssertEqual(
            s.handle(event: .flagsChanged, keyCode: 0x3A, modifierIsDown: true, characters: nil),
            .ignore)  // Left Option down — wait
        let outcome = s.handle(event: .keyDown, keyCode: 0x40, modifierIsDown: false, characters: nil)
        guard case .captured(let chord) = outcome else {
            return XCTFail("F17 with Ctrl+Opt held should capture the chord")
        }
        XCTAssertEqual(chord.keys.map(\.keyCode), [0x3B, 0x3A, 0x40])
        XCTAssertEqual(chord.displayName, "Left Control + Left Option + F17")
    }

    // Modifier-only: press+release Right Option still captures (no main key).
    func testCaptureSessionModifierOnlyOnRelease() {
        var s = HotkeyCaptureSession()
        XCTAssertEqual(
            s.handle(event: .flagsChanged, keyCode: 0x3D, modifierIsDown: true, characters: nil),
            .ignore)
        let outcome = s.handle(event: .flagsChanged, keyCode: 0x3D, modifierIsDown: false, characters: nil)
        guard case .captured(let chord) = outcome else {
            return XCTFail("releasing a lone modifier should capture it")
        }
        XCTAssertEqual(chord, HotkeyChord(key: .rightOption))
    }
}
