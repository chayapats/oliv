...
//
//   • NSPanel(.nonactivatingPanel, level .popUpMenu, ignoresMouseEvents) shown
//     with orderFrontRegardless — never makeKey — so the frontmost app keeps
//     first responder. .canJoinAllSpaces + .fullScreenAuxiliary +
//     .canJoinAllApplications so it rides over other apps' native fullscreen
//     Spaces (iTerm2/herdr, WezTerm, etc.) — .statusBar alone is not enough on
//     modern macOS. Positioned on NSScreen.main (the screen with the focused
//     window) so multi-display users see it where they're looking.
//     hasShadow=false; the SwiftUI pill draws its own material chrome.
//
...
final class RecordingHUDController {
    ...
    /// Re-front the pill when the user switches Spaces while recording so it
    /// follows into a newly activated native-fullscreen terminal Space.
    ...
    func show(phase: HUDPhase) {
        ...
        model.phase = phase
        applyOverlayChrome(panel)
        position(panel)
        if isVisible {
            // Recover from a hide→show race: a stale hide completion can
            // orderOut the panel while isVisible stays true, after which every
            // later show() would early-return and never bring the pill back.
            // Also re-front when already "visible" but on the wrong Space
            // (native fullscreen terminal hosts drop auxiliary panels).
    ...
    /// Completion must not orderOut if show() raced in during the fade —
    /// otherwise the panel leaves the screen while isVisible stays true and
    /// every later press updates phase without ever calling orderFront again.
    func hide() {
    ...
    private func ensurePanel() -> NSPanel {
        ...
        self.panel = panel
        startSpaceObserverIfNeeded()
        return panel
    }

    /// Level + collectionBehavior that let the pill join another app's native
    /// fullscreen Space. Re-applied on every show() — WindowServer can drop
    /// these when the frontmost Space changes under a fullscreen terminal.
    private func applyOverlayChrome(_ panel: NSPanel) {
    ...
    private func startSpaceObserverIfNeeded() {
        ...
        spaceObserver = NSWorkspace.shared.notificationCenter.addObserver(
            ...
            Task { @MainActor in
                self?.refrontIfNeeded()
            }
        }
    }

    /// If we're mid-utterance and the user (or the host terminal) switched into
    /// a native fullscreen Space, pull the pill onto that Space again.
    private func refrontIfNeeded() {
        guard isVisible, let panel = panel else { return }
        applyOverlayChrome(panel)
        position(panel)
        panel.orderFrontRegardless()
