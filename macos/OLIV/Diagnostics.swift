...
enum Diagnostics {
    static func report(
        ...
        thaiFormat: Bool,
        echoCancellation: Bool, duckOtherAudio: Bool,
        ...
        var lines: [String] = [
            ...
            "Thai format: \(onOff(thaiFormat))",
            // The speaker-bleed pair: without these a support report cannot tell
            // "the echo canceller is broken" from "the user turned it off".
            "Echo cancellation: \(onOff(echoCancellation))",
            "Lower other audio: \(onOff(duckOtherAudio))",
