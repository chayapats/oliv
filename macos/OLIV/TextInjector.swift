...
final class TextInjector {
    ...
    /// kVK_ANSI_Z — same layout-independent contract for Cmd+Z (undo).
    private static let keyCodeZ: CGKeyCode = 6
    ...
    /// Overridable for hermetic tests. Default synthesizes Cmd+Z via CGEvent.
    var cmdZPoster: () -> Bool = TextInjector.defaultPostCmdZ
    ...
    /// Outcome of an undo() call — synthesize Cmd+Z in the frontmost app so the
    /// last dictation paste (one undo step in typical text fields) is revoked.
    ...
    /// Post Cmd+Z to the focused app. Same Accessibility / secure-input gates
    /// as inject(): if we can't synthesize keys we return usedFallback and the
    /// caller tells the user to press ⌘Z themselves. Does NOT touch the
    /// pasteboard.
    @discardableResult
    func undo() -> UndoResult {
        if !postAccessCheck() {
            return UndoResult(
                posted: false, usedFallback: true,
                notes: "Accessibility not granted: could not synthesize Cmd+Z.")
        }
        if secureInputCheck() {
            return UndoResult(
                posted: false, usedFallback: true,
                notes: "Secure input is active: a synthesized Cmd+Z would be ignored.")
        }
        if !cmdZPoster() {
            return UndoResult(
                posted: false, usedFallback: true,
                notes: "Cmd+Z synthesis failed unexpectedly despite Accessibility being granted.")
        }
        return UndoResult(posted: true, usedFallback: false, notes: "posted Cmd+Z")
    }
    ...
    private static func defaultPostCmdV() -> Bool {
        postCommandKey(keyCodeV)
    }

    /// Synthesize Cmd+Z (undo) the same way as Cmd+V.
    private static func defaultPostCmdZ() -> Bool {
        postCommandKey(keyCodeZ)
    }
