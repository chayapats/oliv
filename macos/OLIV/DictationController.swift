...
final class DictationController {
    ...
    /// when the undo binding is cleared or overlaps a push-to-talk chord.
    private var undoHotkey: HotkeyMonitor?
    /// Text from the most recent successful auto-paste. Cleared after a
    /// successful undo, or replaced by the next pastedOK. clipboard-only /
    /// fallback pastes do not count — Cmd+Z would not reverse them.
    ...
    /// Which push-to-talk chords the tap listens for (primary, optional
    /// secondary). Change live via `applyHotkeys(_:)` (Settings › General); a
    /// bare set only takes effect at the next `start()`.
    var hotkeyChords: [HotkeyChord] = [.default]

    /// Momentary binding that undoes the last successful paste (Cmd+Z). Nil
    /// disables the undo tap. Change live via `applyUndoHotkey(_:)`.
    var undoHotkeyChord: HotkeyChord?

    /// Primary chord's first key — back-compat for tests/callers.
    var hotkeyKey: HotkeyKey {
        get { hotkeyChords[0].keys[0] }
        set { hotkeyChords = [HotkeyChord(key: newValue)] + hotkeyChords.dropFirst() }
    }
    var hotkeyKeys: [HotkeyKey] {
        get { hotkeyChords.map { $0.keys[0] } }
        set { hotkeyChords = newValue.map { HotkeyChord(key: $0) } }
    }
    ...
    /// Build + start the tap on the current `hotkeyChords`. Idempotent (no-op if
    /// a tap is already live). If tap creation fails (Input Monitoring not yet
    ...
    /// Live-apply push-to-talk chords (Settings › General): tear the current tap
    /// down and install a fresh one. The monitor's stop() fires a balancing
    /// release if a hold was in flight, so no stuck-recording state survives
    /// the swap. No-op when the chord set is unchanged and a tap is live.
    func applyHotkeys(_ chords: [HotkeyChord]) {
        let next = chords.isEmpty ? [HotkeyChord.default] : chords
        if next == hotkeyChords, hotkey != nil { return }
        hotkeyChords = next
        hotkey?.stop()
        hotkey = nil
        installHotkey()
        // PTT change may newly collide with (or free) the undo binding.
        undoHotkey?.stop()
        undoHotkey = nil
        installUndoHotkey()
    }

    /// Single-key convenience (tests / callers that only set primary).
    func applyHotkey(_ key: HotkeyKey) {
        applyHotkeys([HotkeyChord(key: key)])
    }

    /// Live-apply the undo binding. Tears down / reinstalls the undo tap.
    /// Passing nil (or a chord that collides with push-to-talk) disables undo.
    func applyUndoHotkey(_ chord: HotkeyChord?) {
        if chord == undoHotkeyChord, undoHotkey != nil || chord == nil { return }
        undoHotkeyChord = chord
        undoHotkey?.stop()
        undoHotkey = nil
        installUndoHotkey()
    }

    /// Install the momentary undo tap. Skips when unset, or when the chord is
    /// also a push-to-talk binding (both would fire and fight each other).
    private func installUndoHotkey() {
        guard undoHotkey == nil else { return }
        guard let chord = undoHotkeyChord else { return }
        if hotkeyChords.contains(chord) {
            NSLog("OLIV DictationController: undo hotkey \(chord.displayName) "
                + "matches a push-to-talk binding — undo disabled until rebound")
            return
        }
        let monitor = HotkeyMonitor(
            chords: [chord],
            onPress: { [weak self] in
                DispatchQueue.main.async {
                    MainActor.assumeIsolated { self?.handleUndo() }
                }
            },
            onRelease: { /* momentary — undo fires on press only */ },
            // Swallow the letter key while modifiers are held so Ctrl+Shift+Z
            // does not also land in the host app as Redo.
            swallowsMatchedKeyEvents: true
        )
        do {
            try monitor.start()
            undoHotkey = monitor
        } catch {
            NSLog("OLIV DictationController: undo hotkey could not start (\(error))")
        }
    }

    /// Undo the last auto-pasted dictation by synthesizing Cmd+Z. One-shot:
    /// a successful undo clears the remembered text so a second press doesn't
    /// keep eating the user's own edits.
    func handleUndo() {
        // Never interrupt an in-flight utterance — the user is mid-dictate.
        guard appState.status == .idle else { return }
        guard let text = lastPastedText, !text.isEmpty else {
            hud?.notice("Nothing to undo", systemImage: "arrow.uturn.backward")
            return
        }
        let result = injector.undo()
        if result.posted {
            lastPastedText = nil
            hud?.notice("Undid last dictation", systemImage: "arrow.uturn.backward")
        } else {
            hud?.notice("Couldn’t undo — press ⌘Z", systemImage: "exclamationmark.triangle.fill")
        }
    }

    /// Test / diagnostics seam: is there a remembered paste that undo would act on?
    var hasUndoablePaste: Bool {
        guard let text = lastPastedText else { return false }
        return !text.isEmpty
    }
    ...
    /// idle here.
    ///
    /// `immediate` is the QUIT path: the output-volume restore is written
    ...
    private func handleRelease() {
        ...
        Task.detached { [weak self] in
            ...
            await MainActor.run {
                ...
                // Remember only a true auto-paste so undo's Cmd+Z matches what
                // landed at the cursor. pasteNeedsManual left text on the
                // clipboard — Cmd+Z would not reverse it.
                if outcome == .pastedOK, let text = frozenPastedText, !text.isEmpty {
                    controller?.lastPastedText = text
                }
                switch outcome {
