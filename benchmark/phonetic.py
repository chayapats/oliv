...
def correct_with_vocab(text: str, vocab_terms):
    ...
    # Latin ASR garble windows (e.g. user vocab "Terraform" vs STT "Terra form").
    # Only Latin letter-tokens; intervening whitespace OK; first-fold-letter anchor.
    _LATIN_TOK = re.compile(r"^[A-Za-z][A-Za-z0-9]*$")
    latin_idx = [k for k in idx if _LATIN_TOK.match(spans[k][2])]
    for a in range(len(latin_idx)):
        for w in (1, 2, 3):
            if a + w > len(latin_idx):
                break
            pos_ids = latin_idx[a:a + w]
            lo, hi = pos_ids[0], pos_ids[-1]
            # Every non-space token between lo..hi must itself be in this Latin window
            # (rejects "Terra form Kafka" spanning unrelated terms).
            between = [k for k in idx if lo <= k <= hi]
            if between != pos_ids:
                continue
            toks = [spans[i][2] for i in pos_ids]
            start = spans[pos_ids[0]][0]
            end = spans[pos_ids[-1]][1]
            wf = fold("".join(toks))
            if len(wf) < MIN_FOLD_LEN or not word_folds:
                continue
            first = wf[0]
            scored = sorted(((_norm_dist(wf, f), term) for term, f in word_folds))
            d1, best = scored[0]
            if (d1 <= 0.30 and best and fold(best)[0] == first
                    and _consonant_coverage(best, wf) >= CONS_COVERAGE):
                if len(scored) >= 2 and scored[1][0] - d1 < FUZZY_MARGIN:
                    pass
                else:
