...
@register("tooShort_latin_gain_detranslit_passes")
def _():
    # Synthetic: dense Thai syllable soup -> few English jargon tokens.
    # Token count collapses below 0.6x, but Latin-gain marks legitimate restore.
    # Must NOT be discarded as tooShort (R1/R2 still apply separately).
    dict_in = "เปิดออบเซอร์วาบิลิตี้ด้วยโอเป็นเทเลเมทรีย์"
    gen = "เปิด observability ด้วย OpenTelemetry"
    final = _passes(dict_in, gen)
    assert "observability" in final.lower()
    assert "opentelemetry" in final.lower()


@register("tooShort_latin_gain_multi_term_passes")
def _():
    # Another synthetic dense restore: several English terms from Thai garble.
    dict_in = "โหลดบาลานเซอร์กระจายทราฟิกไปหลายอินสเต็นต์ส์"
    gen = "load balancer กระจาย traffic ไปหลาย instance"
    final = _passes(dict_in, gen)
    assert "load balancer" in final.lower() or "balancer" in final.lower()


@register("tooShort_still_fires_without_latin_gain")
def _():
    # Pure truncation to a short Thai fragment: no Latin gain -> still tooShort.
    _fires("ช่วยสรุปประเด็นหลักของการประชุมเมื่อเช้าให้หน่อยนะครับ",
           "สรุป", "tooShort->dict")


@register("tooShort_latin_gain_still_rejects_span_loss")
def _():
    # Latin-gain waives tooShort but not R1: dropping a protected Latin span
    # still falls back even when other jargon is restored.
    dict_in = "เปิด Grafana ด้วยออบเซอร์วาบิลิตี้และโอเป็นเทเลเมทรีย์"
    gen = "เปิด observability ด้วย OpenTelemetry"
    _fires(dict_in, gen, "spanLoss->dict")
...
# ---------------------------------------------------------------------------
# Gate — loan-compound vs clean-thai (synthetic; not eval IDs)
# ---------------------------------------------------------------------------

@register("gate_loan_compound_syllable_soup")
def _():
    # Force hits=0: structural loan shape alone must open the gate.
    # โหลด|บา|ลาน|เซอร์ is the textbook newmm shred of a loan compound.
    run, reason, susp = _gate("โหลดบาลานเซอร์", 0)
    assert run is True, reason
    assert "loan-compound" in reason, reason
    assert _loan_compound_runs("โหลดบาลานเซอร์") == ["โหลดบาลานเซอร์"]
    assert susp == []


@register("gate_loan_compound_observability_shape")
def _():
    run, reason, _susp = _gate("ออบเซอร์วาบิลิตี้", 0)
    assert run is True and "loan-compound" in reason, reason


@register("gate_clean_thai_narrative_prose")
def _():
    text = "ช่วยสรุปประเด็นหลักของการประชุมเมื่อเช้าให้หน่อยนะครับ"
    run, reason, susp = _gate(text, 0)
    assert run is False and reason == "clean-thai", (run, reason, susp)


@register("gate_clean_thai_spoken_numbers")
def _():
    # nm-class: short syllables but spoken-number exclusion must keep clean-thai.
    text = "หนึ่งร้อยยี่สิบสามบาทห้าสิบสตางค์"
    run, reason, _susp = _gate(text, 0)
    assert run is False and reason == "clean-thai", (run, reason)


@register("gate_clean_thai_format_commands")
def _():
    text = "ขึ้นบรรทัดใหม่แล้วทำตัวหนา"
    run, reason, _susp = _gate(text, 0)
    assert run is False and reason == "clean-thai", (run, reason)


@register("gate_keep_thai_tech_not_loan")
def _():
    # Keep-Thai tech words (เช็ค/ล็อก/ไฟล์/พอร์ต) are short but NOT syllable soup.
    for text in ("ช่วยเช็คล็อกในระบบหน่อย", "เปิดไฟล์แล้วเช็คพอร์ต"):
        run, reason, _susp = _gate(text, 0)
        assert run is False and reason == "clean-thai", (text, run, reason)


@register("gate_latin_crumb_opens")
def _():
    # Lone Latin letter amid Thai (p99 → "P นาย") must not clean-thai-skip.
    run, reason, _susp = _gate("ค่า latency P นาย นาย เกิน threshold", 0)
    assert run is True and "latin-crumb" in reason, (run, reason)
...
@register("r1_asr_english_repair_passes")
def _():
    # STT English junk repaired to sound-alike tech English — not spanLoss.
    final = _passes(
        "container นี้ Memory League ต้องโปรไฟล์หา root cost",
        "container นี้ memory leak ต้อง profile หา root cause",
    )
    assert "memory leak" in final.lower()
    assert "root cause" in final.lower()


@register("r1_protected_brand_still_strict")
def _():
    # Soft repair must NOT waive Grafana→Kibana (canonical brand).
    _fires("เปิด Grafana ดู log ครับ", "เปิด Kibana ดู log ครับ", "spanLoss->dict")


@register("translate_gloss_rejects_thai_to_english")
def _():
    # Short Thai verbs becoming English (ใช้→use) — R2 misses; gloss catches.
    _fires("ใช้ Kubernetes จัดการ container ใน cluster",
           "use Kubernetes manage container ใน cluster", "translate->dict")
    # Legitimate new jargon without gloss verbs still passes.
    _passes("เปิดออบเซอร์วาบิลิตี้ด้วยโอเป็นเทเลเมทรีย์",
            "เปิด observability ด้วย OpenTelemetry")
