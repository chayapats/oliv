...
TRANSLIT = {
    ...
    "ทัฟฟิก": "traffic", "ทราฟิก": "traffic",  # Typhoon often writes ทราฟิก; both map to traffic
    "เวิร์คฟอร์มโฮม": "work from home", "สแตนอัพ": "standup",  # mx05
    ...
    "แคมเปญ": "campaign",
    # ---- infra compounds / recurring Typhoon spellings ----
    # Productive patterns attested across multiple eval dumps + existing keys;
    # not one-off leaderboard patches. Longest-first so compounds win.
    "โหลดบาลานเซอร์": "load balancer", "บาลานเซอร์": "balancer",
    "อินสเต็นต์ส์": "instances", "อินสเต็นต์": "instance", "อินสแตนซ์": "instance",
    "เรพิก้า": "replica", "เรพลิกา": "replica",
    "ไมเกรต": "migrate", "ไมเกรท": "migrate",
    "คอนเทนเนอร์": "container",
    "เว็บฮุก": "webhook", "เว็บฮุค": "webhook",
    "รีทราย": "retry",
    "คอนจ๊อป": "cron job", "ครอนจ็อบ": "cron job", "ครอนจอบ": "cron job",
    "รีแฟคเตอร์": "refactor",
    "ออทเทนติเคชัน": "authentication", "ออเธนติเคชัน": "authentication",
    "มิดเดิลแวร์": "middleware",
    "ออบเซอร์วาบิลิตี้": "observability", "ออบเซอร์วาบิลิตี": "observability",
    "โอเป็นเทเลเมทรีย์": "OpenTelemetry", "โอเพนเทเลเมทรี": "OpenTelemetry",
    "กราฟฮานะ": "Grafana",  # Typhoon garble of Grafana (กราฟา already mapped)
    "โพเมทิอุส": "Prometheus", "พรอมิธีอุส": "Prometheus",
    "คราวแฟร์": "Cloudflare", "คราวด์แฟลร์": "Cloudflare",
    "พร้อมเพย์": "PromptPay",
    "ทูมอนี่": "TrueMoney", "ทรูมันนี่": "TrueMoney",
    "บิวท์เฟลล์": "build fail", "บิวเฟล": "build fail",
    "ยูนิตเทสต์": "unit test", "ยูนิตเทส": "unit test",
    "ยูนิตเคส": "unit test",  # Typhoon often hears "test" as เคส; same target
    # Classic multi-source ASR Latin garble (not Thai script) — exact phrase.
    "Engine X": "nginx", "engine x": "nginx", "ENGINE X": "nginx",
    "Memory League": "memory leak", "memory league": "memory leak",
    "root cost": "root cause", "Root Cost": "root cause",
    "Cover eight": "cover edge", "cover eight": "cover edge",
    "check the locks": "check the logs", "Check the locks": "check the logs",
    "Peace review": "please review",
    "รีเวิร์สพ็อกซี่": "reverse proxy", "รีเวิร์สพรอกซี": "reverse proxy",
    "รีเวิร์ส": "reverse", "พ็อกซี่": "proxy", "พรอกซี": "proxy",
    "มองโกดีบี": "MongoDB", "มองโกดีบี้": "MongoDB",
    "คอกโรจดีบี": "CockroachDB", "ค็อกโรชดีบี": "CockroachDB",
    "ฮอริซอนทอลสเกลลิ้ง": "horizontal scaling",
    "ฮอริซอนทอล": "horizontal", "สเกลลิ้ง": "scaling", "สเกลลิง": "scaling",
    # ---- brands / places ----
...
#   ทราฟิก -> traffic         Typhoon spelling variant of ทัฟฟิก (both common)
#   โหลดบาลานเซอร์ -> load balancer   recurring compound; syllables are real Thai
#                            words so gate alone used to skip — dict hits first
#   ออบเซอร์วาบิลิตี้ / โอเป็นเทเลเมทรีย์ -> observability / OpenTelemetry
#                            recurring Typhoon garble shapes for those targets
#
...
CANONICAL_CASE = {
    ...
    "mysql": "MySQL", "mongodb": "MongoDB", "cockroachdb": "CockroachDB",
    "graphql": "GraphQL",
