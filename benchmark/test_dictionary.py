...
@register("pos_traffic_variant_trafik")
def _():
    # ทราฟิก is the productive Typhoon spelling beside ทัฟฟิก (same target).
    out, hits = apply_dictionary("กระจายทราฟิกไปหน่อย")
    assert "traffic" in out and "ทราฟิก" not in out, out
    assert hits == 1, hits


@register("pos_load_balancer_compound")
def _():
    out, hits = apply_dictionary("เปิดโหลดบาลานเซอร์ใหม่")
    assert "load balancer" in out, out
    assert "โหลดบาลานเซอร์" not in out, out
    assert hits == 1, hits
    _no_double_space(out)


@register("pos_opentelemetry_observability_pair")
def _():
    out, hits = apply_dictionary("ใช้โอเป็นเทเลเมทรีย์ดูออบเซอร์วาบิลิตี้")
    assert "OpenTelemetry" in out, out
    assert "observability" in out, out
    assert hits == 2, hits


@register("pos_instance_replica_migrate_cluster")
def _():
    out, hits = apply_dictionary("ไมเกรตเรพิก้าไปอินสเต็นต์ใหม่")
    assert "migrate" in out and "replica" in out and "instance" in out, out
    assert hits == 3, hits


@register("pos_out_of_set_productive_patterns")
def _():
    # Phrases not mirrored from the Wispr-only 19 — productive STT patterns only.
    out, hits = apply_dictionary("ตั้งคราวแฟร์บล็อกทราฟิกแล้วไมเกรตเรพิก้า")
    assert "Cloudflare" in out and "traffic" in out, out
    assert "migrate" in out and "replica" in out, out
    assert hits >= 3, hits
    out2, hits2 = apply_dictionary("ยูนิตเทสยังบิวเฟล")
    assert "unit test" in out2 and "build fail" in out2, out2
    assert hits2 == 2, hits2


@register("pos_engine_x_nginx_and_unit_case")
def _():
    out, hits = apply_dictionary("ตั้งค่า Engine X เป็นรีเวิร์ส พ็อกซี่ให้หน่อย")
    assert "nginx" in out.lower(), out
    assert "reverse" in out.lower() and "proxy" in out.lower(), out
    assert "Engine X" not in out, out
    out2, _ = apply_dictionary("เขียนยูนิตเคส Cover eight เคสให้ครบ")
    assert "unit test" in out2.lower(), out2
    assert "cover edge" in out2.lower(), out2


@register("pos_english_asr_phrase_repairs")
def _():
    out, _ = apply_dictionary("Let me check the locks and get back to you")
    assert "check the logs" in out.lower(), out
    out2, _ = apply_dictionary("container นี้ Memory League หา root cost")
    assert "memory leak" in out2.lower() and "root cause" in out2.lower(), out2


@register("neg_buffalo_not_from_unrelated_thai")
def _():
    # บิวเฟล maps to build fail; plain Thai without that key must stay put.
    out, hits = apply_dictionary("ควายตัวนั้นตัวใหญ่")
    assert out == "ควายตัวนั้นตัวใหญ่" and hits == 0, out
