...
from sidecar_server import (  # noqa: E402
    ...
    _is_silent,
    _strip_cjk,
    _is_implausible_length,
    _is_text_hallucination,
...
# --------------------------------------------------------------------------- #
# W7 gates (C) length-vs-duration + (D) hallucination / repetition
# --------------------------------------------------------------------------- #

@register("length_short_real_thai_passes")
def _():
    # 1s of audio + a normal short Thai line is well under the rate ceiling.
    audio = np.full(16000, 0.1, dtype=np.float32)
    assert _is_implausible_length("ประชุมบ่ายสองนะครับ", audio) is False


@register("length_novel_on_short_clip_rejected")
def _():
    # The spam case: ~1s hold, model dumps a paragraph. 28 chars/s * 1s + slack
    # is ~44, floored to 48 — 200 non-space chars must die.
    audio = np.full(16000, 0.1, dtype=np.float32)
    spam = "ก" * 200
    assert _is_implausible_length(spam, audio) is True
    assert _LENGTH_ABS_FLOOR < 200
    assert _MAX_CHARS_PER_SEC >= 20


@register("length_scales_with_duration")
def _():
    # 30s of real dense Thai at ~20 chars/s (~600 chars) must pass; the same
    # absolute dump on a 1s clip must not.
    long_audio = np.full(16000 * 30, 0.1, dtype=np.float32)
    short_audio = np.full(16000, 0.1, dtype=np.float32)
    body = "ประชุม" * 100  # 600 chars
    assert len(body) == 600
    assert _is_implausible_length(body, long_audio) is False
    assert _is_implausible_length(body, short_audio) is True


@register("length_wav_path_never_gated")
def _():
    # Same policy as the silence gate: benchmark paths are not duration-gated.
    assert _is_implausible_length("ก" * 5000, "/data/clips/hx19.wav") is False


@register("hallucination_caption_phrases")
def _():
    assert _is_hallucination_phrase("Thanks for watching!") is True
    assert _is_hallucination_phrase("please like and subscribe") is True
    assert _is_hallucination_phrase("Thank you for watching. Please subscribe.") is True
    # Real speech that merely mentions a channel must NOT trip the gate.
    assert _is_hallucination_phrase(
        "ช่วยแก้ bug ใน subscribe handler ของ API หน่อย"
    ) is False


@register("hallucination_repetition_spaced_and_thai")
def _():
    assert _is_repetition_loop("ครับ ครับ ครับ ครับ ครับ ครับ") is True
    assert _is_repetition_loop("ครับครับครับครับครับครับ") is True
    assert _is_repetition_loop("you you you you you you") is True
    # Normal polite Thai / short reduplication is fine.
    assert _is_repetition_loop("สวัสดีครับ") is False
    assert _is_repetition_loop("จริงๆ นะครับ") is False


@register("hallucination_combined_predicate")
def _():
    assert _is_text_hallucination("thanks for watching") is True
    assert _is_text_hallucination("ครับ ครับ ครับ ครับ ครับ ครับ") is True
    assert _is_text_hallucination("ประชุมบ่ายสอง") is False


@register("gate_C_handle_too_long_returns_no_speech")
def _():
    # Varied junk (not a pure tile) so gate D's repetition detector does not
    # claim it first — this asserts the length-vs-duration path specifically.
    import sidecar_server as ss
    loud = _pcm(np.full(16000, 0.1))                 # 1s, clears gate A
    spam = "".join(f"x{i % 10}" for i in range(120))  # 240 chars, non-tiling
    assert _is_repetition_loop(spam) is False
    assert _is_implausible_length(spam, np.full(16000, 0.1, dtype=np.float32)) is True
    orig_b, orig_tx = ss._get_backend, ss._transcribe_maybe_redecode
    try:
        ss._get_backend = lambda engine: type("_B", (), {"seed_prompt": False})()
        ss._transcribe_maybe_redecode = lambda *a, **k: (spam, False)
        rep = ss._handle({"cmd": "dictate", "id": "gC2", "cleanup": False, "pcm_b64": loud})
    finally:
        ss._get_backend, ss._transcribe_maybe_redecode = orig_b, orig_tx
    assert rep["no_speech"] is True and rep["final"] == "", rep
    assert rep["gate_reason"] == "too_long", rep


@register("gate_D_handle_caption_returns_no_speech")
def _():
    import sidecar_server as ss
    loud = _pcm(np.full(16000, 0.1))
    orig_b, orig_tx = ss._get_backend, ss._transcribe_maybe_redecode
    try:
        ss._get_backend = lambda engine: type("_B", (), {"seed_prompt": False})()
        ss._transcribe_maybe_redecode = lambda *a, **k: ("Thanks for watching!", False)
        rep = ss._handle({"cmd": "dictate", "id": "gD", "cleanup": False, "pcm_b64": loud})
    finally:
        ss._get_backend, ss._transcribe_maybe_redecode = orig_b, orig_tx
    assert rep["no_speech"] is True and rep["final"] == "", rep
    assert rep["gate_reason"] == "hallucination", rep


@register("gate_D_handle_krub_loop_returns_no_speech")
def _():
    import sidecar_server as ss
    loud = _pcm(np.full(16000, 0.1))
    orig_b, orig_tx = ss._get_backend, ss._transcribe_maybe_redecode
    try:
        ss._get_backend = lambda engine: type("_B", (), {"seed_prompt": False})()
        ss._transcribe_maybe_redecode = (
            lambda *a, **k: ("ครับ ครับ ครับ ครับ ครับ ครับ", False))
        rep = ss._handle({"cmd": "dictate", "id": "gD2", "cleanup": False, "pcm_b64": loud})
    finally:
        ss._get_backend, ss._transcribe_maybe_redecode = orig_b, orig_tx
    assert rep["no_speech"] is True and rep["final"] == "", rep
    assert rep["gate_reason"] == "hallucination", rep
