...
#   (C) drop transcripts that are impossibly long for the capture duration
#       (chars/sec) -- the "spam a novel into the cursor" failure when the model
#       doesn't understand the audio.
#   (D) drop known caption-hallucination phrases + tight repetition loops
#       (e.g. "ครับ ครับ ครับ…", "thanks for watching").
# Checks are pure + cheap. Thresholds are deliberately conservative so
# genuinely quiet / fast speech is not cut; raise _SILENCE_RMS only if silence
# still leaks through in practice.
...
# (C) Length vs duration. Dense Thai (no spaces) can be ~15–20 non-space chars/s
# in real speech; 28 leaves headroom for fast talkers + a small absolute floor so
# a short real utterance is never rate-gated. wav_path / non-ndarray audio skips
# this gate (same policy as the silence gate — eval harness stays untouched).
_MAX_CHARS_PER_SEC = 28.0
_LENGTH_ABS_FLOOR = 48      # always allow at least this many non-space chars
_LENGTH_SLACK = 16          # +chars on top of rate*duration
# (D) Caption-training ghosts. Matched against a punctuation-stripped lowercase
# form; a hit means the WHOLE (or near-whole) transcript is the phrase — not a
# real sentence that merely mentions YouTube.
...
_HALLUCINATION_PHRASE_RATIO = 0.70   # phrase must cover this fraction of the text
_REPETITION_MIN_RUN = 6             # same token/unit this many times in a row → loop
_REPETITION_MIN_CHARS = 12          # ignore tiny strings ("ครับครับ" is fine)
...
def _audio_duration_s(audio, *, sample_rate: int = 16000) -> "float | None":
    """Duration of a mic capture in seconds, or None when the audio is not a
    float array (wav_path / unknown) — callers skip duration-based gates then."""
    import numpy as np
...
def _is_implausible_length(text: str, audio, *, sample_rate: int = 16000) -> bool:
    """True if `text` is impossibly long for how short the capture was.

    Catches the "model didn't understand → pasted a novel" failure without
    punishing long real dictation: the ceiling scales with duration
    (`_MAX_CHARS_PER_SEC`) and only non-space characters count (Thai has no
    spaces, so len(text) alone would under-count English). Non-array audio
    (benchmark wav_path) is never gated.
    """
    dur = _audio_duration_s(audio, sample_rate=sample_rate)
    if dur is None or dur <= 0:
        return False
    n = sum(1 for c in text if not c.isspace())
    if n == 0:
        return False
    limit = max(_LENGTH_ABS_FLOOR, int(dur * _MAX_CHARS_PER_SEC) + _LENGTH_SLACK)
    return n > limit


def _normalize_hallucination_text(text: str) -> str:
    """Lowercase + strip punctuation so caption phrases match across variants."""
    s = text.lower().strip()
...
def _is_hallucination_phrase(text: str) -> bool:
    """True if the transcript is (almost) entirely a known Whisper caption ghost.

    Matches a single dominant phrase, or a stitch of several (e.g. "thanks for
    watching please subscribe") by stripping known phrases and seeing whether
    almost nothing real remains. A real sentence that merely contains a word
    like "subscribe" keeps enough remainder to pass.
    """
    norm = _normalize_hallucination_text(text)
    if len(norm) < 8:
        return False
    for phrase in _HALLUCINATION_PHRASES:
        if norm == phrase:
            return True
        if phrase in norm and len(phrase) / len(norm) >= _HALLUCINATION_PHRASE_RATIO:
            return True
    stripped = norm
    ...
    if not stripped:
        return True
    return len(stripped) / len(norm) <= (1.0 - _HALLUCINATION_PHRASE_RATIO)


def _is_repetition_loop(text: str) -> bool:
    """True if the transcript is a tight repetition loop (spaced or unspaced).

    Spaced: "ครับ ครับ ครับ…" / "you you you you you you".
    Unspaced Thai: "ครับครับครับครับครับครับ". A short polite particle once or
    twice is fine — we need a long run covering most of the string.
    """
    t = text.strip()
    if len(t) < _REPETITION_MIN_CHARS:
        return False

    parts = t.split()
    if len(parts) >= _REPETITION_MIN_RUN:
        run = 1
        for i in range(1, len(parts)):
            if parts[i].casefold() == parts[i - 1].casefold():
                run += 1
                if run >= _REPETITION_MIN_RUN:
                    return True
            else:
                run = 1
        # Majority-same short token (not necessarily consecutive after cleanup).
        from collections import Counter
        counts = Counter(p.casefold() for p in parts)
        top, n = counts.most_common(1)[0]
        if len(top) <= 12 and n >= _REPETITION_MIN_RUN and n / len(parts) >= 0.7:
            return True

    compact = re.sub(r"\s+", "", t)
    if len(compact) >= _REPETITION_MIN_CHARS:
        for unit_len in range(1, 9):
            unit = compact[:unit_len]
            if not unit:
                continue
            # How many times `unit` tiles from the start.
            end = 0
            while compact.startswith(unit, end):
                end += unit_len
            copies = end // unit_len
            if copies >= _REPETITION_MIN_RUN and end / len(compact) >= 0.8:
                return True
    return False


def _is_text_hallucination(text: str) -> bool:
    """Gate (D): caption-phrase denylist or a tight repetition loop."""
    if not text or not text.strip():
        return False
    return _is_hallucination_phrase(text) or _is_repetition_loop(text)


def _no_speech_reply(rid, engine: str, *, t_stt: float = 0.0,
                     gate_reason: str = "no_speech") -> dict:
    ...
    client can simply type nothing. `gate_reason` distinguishes silence / CJK /
    too-long / hallucination for logs without changing the client contract."""
    return {
        "id": rid, "ok": True, "engine": engine, "no_speech": True,
        "raw": "", "final": "", "stt_redecoded": False,
        "t_stt": t_stt, "t_cleanup": 0.0, "cleanup_error": None,
        "llm_ran": False, "gate_reason": gate_reason, "guardrail_flag": "",
...
def _handle(req: dict) -> "dict | None":
    ...
    if cmd == "dictate":
        ...
        # W7 gates (C)+(D): after CJK strip, before cleanup — reject spam-length
        # transcripts and known caption / repetition ghosts so they never reach
        # the cursor (and never burn a cleanup LLM call). `raw` above already
        # captured the true STT output for debug; the client still sees empty
        # final + no_speech like the other gates.
        if text.strip() and _is_text_hallucination(text):
            return _no_speech_reply(rid, engine, t_stt=t_stt,
                                    gate_reason="hallucination")
        if text.strip() and _is_implausible_length(text, audio):
            return _no_speech_reply(rid, engine, t_stt=t_stt,
                                    gate_reason="too_long")
